...
const item = { id: itemId, name: 'Demo item', totalQuantity: 10, availableQuantity: 10, heldQuantity: 0, confirmedQuantity: 0, createdAt: '2026-08-19T00:00:00.000Z' };
...
describe('API contract', () => {
  it('creates items and validates bodies', async () => {
    expect((await request(app).post('/v1/items').send({ name: 'Demo item', initial_quantity: 10 })).status).toBe(201);
    const invalid = await request(app).post('/v1/items').send({ name: '', initial_quantity: 0 });
    expect(invalid.status).toBe(400);
    expect(invalid.body.error.code).toBe('VALIDATION_ERROR');
  });

  it('retrieves inventory and validates identifiers', async () => {
    expect((await request(app).get(`/v1/items/${itemId}`)).body).toEqual(item);
    expect((await request(app).get('/v1/items/not-a-uuid')).status).toBe(400);
  });

  it('accepts the assignment request shape and an optional idempotency key', async () => {
    const body = { item_id: itemId, customer_id: 'customer-1', quantity: 2, expires_at: '2099-01-01T00:00:00Z' };
    expect((await request(app).post('/v1/reservations').send(body)).status).toBe(201);
    const created = await request(app).post('/v1/reservations').set('Idempotency-Key', 'request-1').send(body);
  ...
  it('returns 200 for creation replay and maps expected creation failures', async () => {
    ...
    expect((await request(app).post('/v1/reservations').set('Idempotency-Key', 'replay').send(body)).status).toBe(200);
    expect((await request(app).post('/v1/reservations').set('Idempotency-Key', 'missing').send(body)).status).toBe(404);
    expect((await request(app).post('/v1/reservations').set('Idempotency-Key', 'insufficient').send(body)).body.error.code).toBe('INSUFFICIENT_INVENTORY');
    expect((await request(app).post('/v1/reservations').set('Idempotency-Key', 'conflict').send(body)).body.error.code).toBe('IDEMPOTENCY_CONFLICT');
  });

  it('confirms, cancels, and expires reservations', async () => {
    expect((await request(app).post(`/v1/reservations/${reservationId}/confirm`)).body.status).toBe('confirmed');
    expect((await request(app).post(`/v1/reservations/${reservationId}/cancel`)).body.status).toBe('cancelled');
    expect((await request(app).post('/v1/maintenance/expire-reservations').send({ limit: 10 })).body).toEqual({ expiredCount: 0, hasMore: false });
    expect((await request(app).post('/v1/maintenance/expire-reservations').send({ limit: 1001 })).status).toBe(400);
  });
  ...
  it('maps lifecycle not-found, expired, and state conflicts', async () => {
    ...
    expect((await request(app).post(`/v1/reservations/${reservationId}/confirm`)).status).toBe(404);
    expect((await request(app).post(`/v1/reservations/${reservationId}/confirm`)).body.error.code).toBe('RESERVATION_EXPIRED');
    expect((await request(app).post(`/v1/reservations/${reservationId}/confirm`)).body.error.code).toBe('RESERVATION_STATE_CONFLICT');
  });
  ...
  it('documents every applicable response status', () => {
    ...
    expect(Object.keys(paths['/v1/reservations'].post.responses)).toEqual(expect.arrayContaining(['200', '201', '400', '404', '409', '500']));
    expect(Object.keys(paths['/v1/reservations/{reservationId}/confirm'].post.responses)).toEqual(expect.arrayContaining(['200', '400', '404', '409', '500']));
  });
